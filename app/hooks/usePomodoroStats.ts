...
export function usePomodoroStats({ userId, weekEndDate, runWithAuthRetry }: UsePomodoroStatsConfig) {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<PomodoroWeeklyStats> => {
      const params: Record<string, string> = { user_telegram_id: userId! };
      ...
      if (error || !data || !Array.isArray(data) || data.length === 0) {
        return { totalPomodoros: 0, totalFocusMinutes: 0, totalFocusHours: 0, days: [] };
      }
  ...
  return query.data ?? { totalPomodoros: 0, totalFocusMinutes: 0, totalFocusHours: 0, days: [] };
}
