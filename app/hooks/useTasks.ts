...
const isDateInMonth = (value: string | Date | null | undefined, monthKey: string) => {
...
export function useTasks(config: UseTasksConfig) {
  ...
  const flushPendingMutation = useCallback(
    async (tempId: string, id: string) => {
      ...
      if (error) console.error('Flush pending task mutation failed', error);
    },
    [runWithAuthRetry],
  );
  ...
  const ensureSeriesInstancesForMonth = useCallback(
    async (
      ...
      if (rows.length > 0) {
        ...
        if (error) console.error('Series instance upsert failed', error);
      }
    },
    [userId, runWithAuthRetry],
  );
    ...
    queryFn: async () => {
      ...
      if (pendingIds.size > 0) {
        ...
        const pendingTasks = prev.filter(
          (task) =>
            pendingIds.has(task.id) && isDateInMonth(task.date, activeMonthKey),
        );
      ...
      tasksData.forEach((row: TaskRow) => {
        ...
        if (row.series_id) existingKeys.add(`${row.series_id}:${rowDate}`);
      });
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            if (!row?.id || !isDateInMonth(row.date, activeMonthKey)) return;
            const rowPosition = Number(row.position ?? 0);
  ...
  const toggleTaskMutation = useMutation({
    ...
    onMutate: async ({ id, newStatus }: { id: string; newStatus: boolean }) => {
      const snapshot = getTasksSnapshot();
      const target = snapshot.find((t) => t.id === id);
  ...
  const toggleTask = useCallback(
    ...
    [getTasksSnapshot, setTasksCache, queuePendingMutation, toggleTaskMutation],
  );
  ...
  const addTask = useCallback(
    async (
      ...
      currentSnapshot.forEach((task) => {
        ...
        if (task.seriesId) existingKeys.add(`${task.seriesId}:${dateKey}`);
      });
  ...
  const updateTask = useCallback(
    ...
    [getTasksSnapshot, setTasksCache, queuePendingMutation, runWithAuthRetry],
  );
