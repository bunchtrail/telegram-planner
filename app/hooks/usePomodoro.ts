...
export function usePomodoro({ taskId, runWithAuthRetry }: UsePomodoroConfig) {
  ...
  const logSession = useCallback(
    async (type: PomodoroPhase) => {
      const durationMinutes = type === 'focus' ? 25 : type === 'short_break' ? 5 : 15;
  ...
  const advancePhase = useCallback(() => {
    setState((prev) => {
      ...
      const newTotalPomodoros = wasFocus ? prev.totalPomodoros + 1 : prev.totalPomodoros;
      ...
      if (wasFocus) {
        ...
        nextPhase = 'focus';
        if (prev.phase !== 'focus') {
          nextRound = prev.round < POMODOROS_BEFORE_LONG_BREAK ? prev.round + 1 : 1;
        }
      }
